...
When working with JAX-RS, its resource classes are usually created again for each new request. This means that the server should normally use a new object when a client sends a request.
...
Because the data is required to be constantly available, it should also be handled carefully as creating multiple requests could attempt to change it at the same time. If this potential issue is not handled properly, errors or inconsistent results could appear.
